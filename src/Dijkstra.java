...
public class Dijkstra {
    ...
    public void ReadTextfileToBuildGraph() {
        try {
            ...
            System.out.println("Enter the network topology matrix:");

            DistGrph = new int[numRouters][numRouters];

            for (int i = 0; i < numRouters; i++) {
                for (int j = 0; j < numRouters; j++) {
                    DistGrph[i][j] = scanner.nextInt();
                }
            }

            Max_rooters = numRouters;


            System.out.println("<=======:Graph Table Initialized:=======>");
            // Print the network topology matrix
            for (int i = 0; i < numRouters; i++) {
                for (int j = 0; j < numRouters; j++) {
                    System.out.print(DistGrph[i][j] + " ");
                }
                System.out.println();
            }
            System.out.println("==========================================");
